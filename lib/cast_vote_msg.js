...
module.exports = function make_cast_vote(options) {
  async function cast_vote(msg, reply) {
    try {
      const vote_type = msg.vote
      const safe_params = await validateMessage(msg)
      const poll_id = safe_params.fields.poll_id
      const voter_id = safe_params.fields.voter_id
      const voter_type = safe_params.fields.voter_type
      const vote_kind = safe_params.fields.kind
      const vote_code = safe_params.fields.code
      ...
      if ('dependents' in msg) {
        const dependents = msg.dependents
  ...
  async function castVote(args, ctx, options = {}) {
    ...
    const voter_id = args.voter_id
    const voter_type = args.voter_type
    const poll_id = args.poll_id
    const vote_type = args.vote_type
